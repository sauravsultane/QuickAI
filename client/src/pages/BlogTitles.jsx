...
const BlogTitles = () => {
  ...
  const onSubmitHandler = async (e) => {
    e.preventDefault();
    // Title generation logic will go here
  };
  ...
  return (
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        ...
        <button
          type="submit"
          className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#c341f6] to-[#8e37be] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer"
        >
          <Hash className="w-5" />
          Generate Title
        </button>
      ...
      <div className="w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-69">
        <div className="flex items-center gap-3">
          <Hash className="w-5 h-5 text-[#8e37be]" />
